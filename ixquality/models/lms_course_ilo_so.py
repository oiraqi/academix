...
class LmsCourseIloSO(models.Model):
    _name = 'ixquality.lms.course.ilo.so'
    _description = 'LMS Course ILO & SO Mapping'
    _sql_constraints = [('ilo_so_course_program_ukey', 'unique(course_program_id, lms_course_ilo_id, so_id)', 'Mapping already exists')]

    lms_course_ilo_id = fields.Many2one(comodel_name='ixlms.course.ilo', string='ILO', required=True)    
