		...
		for rec in self:
			...
			if rec.grade_grouping == 'module' and rec.module_ids:
				...
				for module in rec.module_ids:
					...
					if s != 100:
						raise ValidationError(f'The sum of percentages of assessments under {module.name} shall be equal to 100%, not {s}%')
			...
			if rec.grade_grouping == 'technique' and rec.technique_ids:
				...
				for technique in rec.technique_ids:
					...
					if s != 100:
						raise ValidationError(f'The sum of percentages of assessments under {technique.name} shall be equal to 100%, not {s}%')
